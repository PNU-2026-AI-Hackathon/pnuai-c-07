...
function IntroduceSection() {
  return (
    <div className="w-full h-screen min-h-screen bg-[#F4FBFB] flex flex-col items-center justify-center text-center px-6">
      
      <h1 className="text-[60px] md:text-5xl lg:text-6xl font-extrabold text-[#292929] mb-8 leading-tight tracking-tight">
        아직도 아무 상품이나 <br className="md:hidden" /> 가입하세요?
      </h1>

      <p className="text-lg md:text-2xl font-semibold text-[#03BFA5] leading-relaxed max-w-3xl">
        30초 만에 원하는 금융상품을 찾는 청년 금융 소울메이트, <span className="font-black">y-Fin.</span>
      </p>
    </div>
    );
};
 ...
 function StepIntroSection() {
  return (
    <section className="w-full h-screen min-h-screen bg-[#FFFFFF] py-24 md:py-32 flex justify-center items-center">
      <div className="text-center px-6">
        <h2 className="text-[32px] md:text-[44px] font-extrabold text-[#454545] leading-tight tracking-tight">
          단 <span className="text-[#03BFA5]">세 단계</span>로 나에게 맞는 금융상품 확인해요.
        </h2>
        
        {/* 시각적 요소를 추가하고 싶다면 아래처럼 아주 연한 언더라인 효과를 줄 수 있습니다. */}
        <div className="mt-4 w-20 h-1.5 bg-[#03BFA5]/20 mx-auto rounded-full" />
      </div>
    </section>
  );
}
  ...
  return (
    <section className="py-24 bg-white flex flex-col items-center">
      ...
      <div className="text-center mb-16 px-4">
        <h2 className="text-[40px] md:text-[40px] font-extrabold text-[#454545] leading-tight">
          왜 이 상품이 나에게 최적인지, <br />
      ...
      {/*카드 그리드*/}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl w-full px-6 items-stretch">
        
        {/*왼쪽 - 사용자 프로필 */}
        <div className="bg-[#F9FAFB] rounded-[3rem] p-10 flex flex-col">
        ...
        {/*중앙 - 매칭 점수*/}
        <div className="relative bg-white rounded-[3rem] p-12 flex flex-col items-center justify-center shadow-[0_20px_50px_rgba(226,226,226,0.7)] transform lg:scale-105 z-10">
        ...
        {/*추천상품 상세*/}
        <div className="bg-[#F8FAFB] rounded-[3rem] p-10 flex flex-col">
          <h3 className="text-[17px] text-center text-sm font-bold text-[#454545] mb-8">
...
function ProfileItem({ text }) {
  return (
    <div className="bg-white px-6 py-5 rounded-[1.2rem] shadow-sm flex items-center gap-4 mb-4 transition-all hover:translate-x-1">
      ...
      <span className="text-[17px] font-bold text-[#454545] tracking-tight">
        {text}
      </span>
    </div>
  );
...
function MatchBadge({ title, sub, matched }) {
  return (
    <div className={`px-5 py-4 rounded-2xl border transition-all ${matched ? 'bg-[#FFFFFF] border-transparent shadow-sm' : 'bg-[#FFFFFF] border-transparent shadow-sm '}`}>
  ...
  return (
    <div className="flex items-start gap-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-2">
        ...
        <p className="text-[17px] font-medium text-[#666] leading-relaxed">
          {content}
        </p>
      </div>
    </div>
...
function Tag({ text, icon }) {
  return (
    <div className={`
      px-5 py-2.5 rounded-full border border-gray-200 bg-white shadow-sm flex items-center gap-2
 ...
 function ChatbotSection() {
  return (
    <section className="w-full h-screen min-h-screen bg-[#FFFFFF] py-24 md:py-32 flex justify-center items-center">
      <div className="text-center px-6">
        <h2 className="text-[32px] md:text-[44px] font-extrabold text-[#454545] leading-tight tracking-tight">
          어려운 금융 용어는<p></p><span className="text-[#03BFA5]">챗봇 AI한테</span> 바로바로 물어보세요.
        </h2>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-24 bg-white flex flex-col items-center px-6">
      ...
      <div className="text-center mb-20">
        <h2 className="text-[32px] md:text-[40px] font-extrabold text-[#333] leading-tight">
          혼자 고민하지 마세요. <br />
...
function ProductCard({ agency, badges, subTitle, title, desc, rate, subDesc, isHighlighted, buttonText }) {
  return (
    <div className={`
      relative p-10 rounded-[2.5rem] border-2 transition-all flex flex-col h-full
      ${isHighlighted ? 'border-[#2DD4BF] bg-[#EFFFFD]' : 'border-[#E2E8F0] bg-[#FFFFFF] shadow-sm'}
    `}>
      {/*기관 및 뱃지 */}
      <div className="flex justify-between items-center mb-10">
        <span className="text-[15px] font-inter text-[#454545]">{agency}</span>
        <div className="flex gap-2">
          {badges.map((badge, i) => (
            <span key={i} className="px-3 py-1 bg-[#2DD4BF] text-white text-[12px] font-bold rounded-full">
              {badge}
            </span>
          ))}
        </div>
      ...
      <div className="mb-10 flex-1">
        ...
        <p className="text-[13px] text-gray-500 font-semibold leading-relaxed whitespace-pre-wrap">
          {subDesc}
        </p>
      </div>

      {/* 하단 버튼/링크 */}
      <div className="pt-8 border-t border-gray-100 flex justify-between items-center group cursor-pointer">
        <span className="text-[16px] font-bold text-[#454545]">{buttonText}</span>
        <svg 
          className={`w-8 h-8 transition-transform ${isHighlighted ? 'text-[#2DD4BF]' : 'text-gray-300'}`} 
  ...
  return (
    <section className="py-24 bg-white flex flex-col items-center px-6">
      ...
      <div className="text-center mb-16">
        ...
        {/* 매거진 카드 그리드 */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 max-w-7xl w-full">
...
function MagazineCard({ title, imgUrl }) {
  return (
      <div className="relative overflow-hidden rounded-4xl border-[1.5px] border-[#B6D6D2] shadow-sm transition-all duration-300">
        {/* 이미지 비율 고정 */}
        ...
        {/* 하단 텍스트 영역 */}
        <div className="bg-white p-6 md:p-8 border-t border-[#B6D6D2]/10">
          <h3 className="text-[22px] md:text-[19px] font-semibold text-[#454545] leading-snug whitespace-pre-wrap">
            {title}
          </h3>
        </div>
      </div>
...
function MyPageSection() {
  return (
    <section className="w-full h-screen min-h-screen bg-[#FFFFFF] py-24 md:py-32 flex justify-center items-center">
      <div className="text-center px-6">
        <h2 className="text-[32px] md:text-[44px] font-extrabold text-[#454545] leading-tight tracking-tight">
          당신이 입력한 정보,<p></p><span className="text-[#03BFA5]">마이페이지</span>에 저장하고 다음 추천에 활용해보세요.
        </h2>
      </div>
    </section>
  );
